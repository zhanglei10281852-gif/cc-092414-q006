...
- 证书复核：第三方实验室合格证书按草稿、送审、退回、复核通过、撤销流转，录入人与复核人必须分离，接近限值的结果强制双人复核并留存意见、证书版本与批次风险快照；撤销已引用证书会触发批次重新评估而非静默删除。
...
测试覆盖身份初始化、登录、用户与角色维护、权限计算、账号停用后的会话撤销、审计脱敏、农产品批次、抽样检测、运输追踪、风险任务、证书复核流转（双人复核、接近限值意见、版本幂等、撤销再评估）和数据库时间格式。
